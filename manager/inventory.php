...
if (isset($_POST['add_product'])) {
    ...
    if (in_array($imageFileType, $allowed_types) && move_uploaded_file($_FILES["prod_image"]["tmp_name"], $target_file)) {
        ...
        $stmt = $con->prepare("INSERT INTO tblproducts (prod_image, ProductName, description, price, ProductTypeID, ExpirationDate, ProductCode) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssdsss", $prod_image, $ProductName, $description, $price, $ProductTypeID, $ExpirationDate, $ProductCode);
...
<body class="cbp-spmenu-push">
    ...
    <div class="pull-right">
        <button type="button" class="btn" style="background-color: #337ab7; color: #fff;" data-toggle="modal" data-target="#addProductModal">Add New Product</button>
    </div>
</form>
...
$query .= " ORDER BY tblproducts.ProductID DESC";
        ...
        ?>
            <tr>
                ...
                <td>
                    <a href="#" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#editModal" data-id="<?php echo $row['ProductID']; ?>" data-name="<?php echo $row['ProductName']; ?>" data-description="<?php echo $row['description']; ?>" data-price="<?php echo $row['price']; ?>" data-qty="<?php echo $row['Quantity']; ?>" data-unit="<?php echo $row['unit']; ?>" data-type="<?php echo $row['ProductTypeID']; ?>" data-expirationdate="<?php echo $row['ExpirationDate']; ?>">
                        Edit
                    </a>
                </td>
            </tr>
